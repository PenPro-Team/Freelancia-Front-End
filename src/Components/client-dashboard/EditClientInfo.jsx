...
export default function EditClientInfo(props) {
  ...
  useEffect(() => {
    ...
    AxiosUserInstance.get(`${params.user_id}`)
      .then((res) => {
        ...
        console.log("User Data from api", res.data.image);
  ...
  const handleSubmit = (e) => {
    let sendingData = { ...formValues };
    e.preventDefault();
    setShowSuccess(false);
    setShowError(false);
    if (isFormValid) {
      if (sendingData.image) {
        delete sendingData.image;
      }
      // console.log("Sending Data:", sendingData);
      AxiosUserInstance.patch(`${params.user_id}`, sendingData, {
        headers: { Authorization: `Bearer ${user.access}` },
      })
        .then((res) => {
          setShowSuccess(true);
          setUserData(res.data);
          console.log("User Data from api in Update Page", res.data);
          console.log("User Image Data from api", res.data.image);
          props.cb();
        })
        .catch((error) => {
          console.error(error);
